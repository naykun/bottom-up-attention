...
# Example:
# python ./tools/generate_tsv_gt_SG.py --cfg experiments/cfgs/faster_rcnn_end2end_resnet.yml --def models/vg/ResNet-101/faster_rcnn_end2end_final/test_gt.prototxt --out feature/VG/VG_gt_resnet101_faster_rcnn_genome.tsv --net data/faster_rcnn_models/resnet101_faster_rcnn_final.caffemodel --total_group 1 --group_id 0 --split visgenome_gt
...
def load_image_ids(split_name, group_id, total_group, image_ids=None):
    ...
    if split_name == 'coco_train2014':
      ...
      for metadata_p in metadata_path:
        metadata_fn = json.load(open(os.path.join('data/VCR/vcr1images', metadata_p), 'r'))
      ...
      data_root = '/app/data/VG_100K/VGdata_raw/image_data.json'
      ...
      with open(data_root) as f:
        ...
        object_id = np.zeros([len(data),max_num_box])
        for i, image in enumerate(data):
          num_bbox[i] = len(image['objects'])
          for j, object_ in enumerate(image['objects']):
            object_id[i,j] = object_['object_id']
            bbox[i,j,1] = object_['x']
            bbox[i,j,2] = object_['y']
            bbox[i,j,3] = object_['x'] + object_['w'] 
            bbox[i,j,4] = object_['y'] + object_['h']
          image_id = image['image_id']
          # image_name = img_info[image_id-1]['url'].split('/')[-2:]
          file_path = os.path.join('/app/data/VG_100K/images/VG_100K', str(image_id) + '.jpg')
          split.append((file_path,image_id))

      total_num = len(split)
      per_num = int(np.ceil(total_num / total_group))
      split = split[int(group_id * per_num):int((group_id+1)*per_num)]
      object_id = object_id[int(group_id * per_num):int((group_id+1)*per_num)]
...
if __name__ == '__main__':
    ...
    generate_tsv(1, args.prototxt, args.caffemodel, image_ids, bbox, num_bbox, outfile)
